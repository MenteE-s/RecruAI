...
- 2026-09-20: MEDIUM/LOW all done + tested. TIME/DATE overhaul done + tested: backend Z timestamps (utc_iso/utc_now_iso, 43 models + API layer), 2 TypeError crash fixes, join_date parse, UTC date.today, interview org/candidate timezones in payload, frontend parseUTC + DualTime side-by-side rollout (7 interview screens), deadline worldwide fix, selector hydrate + save-URL fix, dual-zone notification text. Uncommitted — awaiting push decision.
- 2026-09-20: Stuck-status fix: lazy auto-complete of overdue interviews on detail/list/upcoming reads (is_overdue/complete_overdue_interview, tested: stuck/future/terminal/None-duration). Root cause was disabled scheduler + crash-broken expiry sweep; scheduler stays off, reads now self-heal.
- 2026-09-20: Text-interview both-sides E2E proven live (mocked AI, self-cleaning fixtures): candidate 5/5, org 4/4, contractor detail ok but chat+conversation 403 → fixed ai_chat + conversation auth to TeamMember-aware (same rule as detail), re-tested 200/200. Uncommitted.
